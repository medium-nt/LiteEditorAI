...
let screenFrame = () => null;  // (sid) → {cols,rows,lines[],cursor:[x,y],alt} — задаётся из main
let writeInput = () => {};
let openProject = () => {};
let onSelect = () => {};
let onClose = () => {};
...
function flushScreen(forceFull) {
  ...
  if (full) {
    frameSeq++;
    send({ t: 'screen', sid, seq: frameSeq, full: true, cols: f.cols, rows: f.rows, alt: !!f.alt, cursor: f.cursor, lines: f.lines });
  } else {
    const diff = [];
    ...
    frameSeq++;
    send({ t: 'screen', sid, seq: frameSeq, alt: !!f.alt, cursor: f.cursor, diff });
  }
  lastFrame = { sid, cols: f.cols, rows: f.rows, lines: f.lines, cursor: f.cursor, alt: !!f.alt };
}
