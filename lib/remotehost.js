...
function execCapture(c, cmd, timeoutMs) {
...
const SCAN_CMD =
  ...
  'echo __SOCKS__; ' +
  'test -S /var/run/docker.sock && echo docker=/var/run/docker.sock; ' +
  'test -S /run/podman/podman.sock && echo podman=/run/podman/podman.sock; ' +
  'test -S "/run/user/$(id -u)/podman/podman.sock" && echo "podman=/run/user/$(id -u)/podman/podman.sock"; true';
async function scanServices(c) {
  ...
  for (const line of String(socksPart || '').split('\n')) {
    const m = line.trim().match(/^(docker|podman)=(.+)$/);
    if (m && !sockets[m[1]]) sockets[m[1]] = m[2];
  }
  return { ok: true, services, sockets };
}
...
function openTunnelGeneric(c, label, connectStream) {
  return new Promise((resolve) => {
    const ssh = new SshClient();
    let done = false;
    ...
    ssh.on('ready', () => {
      ...
      server.on('error', (e) => fin({ ok: false, error: String(e.message || e) }));
      server.listen(0, '127.0.0.1', () => {
        const tunId = 'tun' + Date.now().toString(36) + Math.floor(Math.random() * 1e5).toString(36);
        const t = { ssh, server, port: server.address().port, connId: c.id, connName: c.name || c.host, label };
        tunnels.set(tunId, t);
...
async function openTunnel(c, rhost, rport, label) {
  const r = await openTunnelGeneric(c, label || `${rhost}:${rport}`, (ssh, cb) => ssh.forwardOut('127.0.0.1', 0, rhost, rport, cb));
...
function registerRemoteIpc({ ipcMain, safeStorage, send, getConnections, setConnections, onSessionOpen }) {
  ...
  ipcMain.handle('rh:tunnelOpen', async (_e, { id, rhost, rport, label } = {}) => {
    const c = loadConns().find((x) => x.id === id); if (!c) return { ok: false, error: 'Подключение не найдено' };
    ...
    return await openTunnel(c, rhost || '127.0.0.1', port, label);
  });
