...
//
// main.js wires this: registerRemoteIpc({ ipcMain, safeStorage, dialog, send, getConnections, setConnections }).
const fs = require('fs');
const os = require('os');
const path = require('path');
const { Client: SshClient } = require('ssh2');

let _safe = null, _get = null, _set = null, _dialog = null, _send = null;
...
// ---------------------------------------------------------------- IPC
function registerRemoteIpc({ ipcMain, safeStorage, dialog, send, getConnections, setConnections }) {
  _safe = safeStorage; _dialog = dialog; _send = send; _get = getConnections; _set = setConnections;
  ...
  ipcMain.handle('rh:open', async (_e, { sessionId, id, cols, rows } = {}) => {
  ...
  // Pick a private key file → return its text (stored encrypted on save). Renderer never holds the path long-term.
  ipcMain.handle('rh:pickKey', async () => {
    const r = await _dialog.showOpenDialog({ properties: ['openFile'], title: 'Выбрать приватный SSH-ключ' });
