...
| 📋 **IterFlow** | Задачи из таск-трекера [IterFlow](https://iter-flow.ru) — прямо в редакторе. Вход → выбор профиля → заказчика → проекта, дальше вкладки **Итерация** (таймлайн итераций с задачами), **Канбан** (плоский, со сменой статуса), **Туду** и **Чат**. Задачи показываются строкой-заголовком с подсветкой по важности, клик раскрывает детали. Удобно фрилансерам и студиям, которые согласуют работу с заказчиком: берите задачи в работу с ИИ-агентом, не уходя в браузер. |
