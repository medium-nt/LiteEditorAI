...
| ⎇ **Git** | В стиле JetBrains: ветки и переключение, изменения с диффом по клику, commit / push / pull / fetch, **stash и возврат**, откат файла или всех правок, **история коммитов**. Статус — прямо в дереве файлов. |
| 🧠 **Контекст** | Сборка **CLAUDE.md / AGENTS.md как графа** (в духе n8n): блоки-источники (текст, файл, команда, память агента) → выходы Claude / Codex, группы-профили по тумблеру, несколько графов на проект, счётчики ≈токенов. Готовый CLAUDE.md можно **распилить на блоки локальным агентом**; перед перезаписью — бекап. |
